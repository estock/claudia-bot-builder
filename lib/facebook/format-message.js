...
class Media extends FacebookTemplate {
    constructor() {
        super();
    ...
    addBubble(title, subtitle) {
        if (this.bubbles.length === 1)
            throw new Error('1 bubble is maximum for Media template');

        if (title)
            throw new Error('Bubbles on media templates do not have a Title');

        if (subtitle)
            throw new Error('Bubbles on media templates do not have a Subtitle');
    ...
    addImage(url) {
        ...
        this.getLastBubble()['media_type'] = 'image';
        this.getLastBubble()['url'] = url;
    ...
    addVideo(url) {
        if (!url)
            throw new Error('Video URL is required for addVideo method');

        if (!isUrl(url))
            throw new Error('Video URL needs to be valid for addVideo method');

        this.getLastBubble()['media_type'] = 'video';
        this.getLastBubble()['url'] = url;
